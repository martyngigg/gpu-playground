...
__global__ void pathcalc(float *d_z, float *d_v)
{
  ...
  printf("blockIdx offsets : %d  %d  %d\n", blockIdx.x, threadIdx.x + 2*N*blockIdx.x*blockDim.x, 2*N*threadIdx.x + 2*N*blockIdx.x*blockDim.x);
...
int main(int argc, const char **argv){
  ...
  h_z = (float *)malloc(sizeof(float)*2*h_N*NPATH);
  ...
  checkCudaErrors( cudaMemcpy(h_z, d_z, 2*h_N*NPATH,
